...
export default function useJob() {
  const resolveJobStatus = (job) => {
    ...
    else if (moment().valueOf() > moment(job.end).valueOf()) return { text: i18n.global.t('layout.navbar.helper.job.status.expired'), value: 'expired', card: 'bg-soft-success', badge: 'bg-success' };
