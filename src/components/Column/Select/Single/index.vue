<template>
  ...
  <div v-else>
    <div v-if="value && value.text && value.tid && value.rid" class="cursor-pointer text-decoration-underline text-secondary" @click="handleClickValue(value)" :title="`${value.tid}/${value.rid}`">
      {{ value.text }}
    </div>
