<template>
  <div class="row g-3">
    ...
    <div class="col-md-3 col-sm-6">
      <label class="form-label">{{ $t('layout.navbar.helper.form.column.config.dateFormat') }}</label>
      <VueSelect
        v-model="column.cfg.dateFormat"
        :placeholder="$t('layout.navbar.helper.form.column.config.dateFormat')"
        :options="['Y-m-d H:i:S', 'Y-m-d H:i', 'Y-m-d', 'H:i:S', 'H:i']"
        :clearable="false"
        @option:selected="
          () => {
            column.type = ['Y-m-d H:i:S', 'Y-m-d H:i'].includes(column.cfg.dateFormat) ? 'datetime' : ['Y-m-d'].includes(column.cfg.dateFormat) ? 'date' : 'time';
          }
        "
      >
    ...
    <div class="col-md-6">
      ...
      <input v-model="column.default" type="text" :placeholder="$t('layout.navbar.helper.form.column.config.default')" :class="['form-control', errors.default && 'is-invalid']" />
      <Field name="default" v-model="column.default" class="d-none" :class="{ 'is-invalid': errors.default }" rules="exclude:*==" />
      <span class="invalid-feedback">{{ errors.default }}</span>
    </div>

    <div v-if="['Y-m-d H:i:S', 'Y-m-d H:i', 'Y-m-d'].includes(column.cfg.dateFormat)" class="col-md-6">
      <label class="form-label">{{ $t('layout.navbar.helper.form.column.config.minDate') }}</label>
      <input v-model="column.cfg.minDate" type="text" :placeholder="$t('layout.navbar.helper.form.column.config.minDate')" class="form-control" />
    </div>

    <div v-if="['Y-m-d H:i:S', 'Y-m-d H:i', 'Y-m-d'].includes(column.cfg.dateFormat)" class="col-md-6">
      <label class="form-label">{{ $t('layout.navbar.helper.form.column.config.maxDate') }}</label>
      <input v-model="column.cfg.maxDate" type="text" :placeholder="$t('layout.navbar.helper.form.column.config.maxDate')" class="form-control" />
    </div>
