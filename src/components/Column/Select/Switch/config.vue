<template>
  <div class="row g-3">
    ...
    <div class="col-md-12">
      ...
      <VueSelect v-model="column.tags" multiple :close-on-select="false" taggable push-tags :placeholder="$t('layout.navbar.helper.form.column.config.tags')" :options="['cellEdit', 'batchUpdate']">
        <template v-slot:no-options="{ search, searching }">
          <template v-if="searching">
            <span v-html="$t('components.vs.search', { search })"></span>
          </template>
