...
export const getYearWeeks = (y) => {
  y = y || new Date().getFullYear();
  const dates = [];
  for (let m = 1; m <= 12; m++) {
    for (let d = 1; d <= new Date(y, m, 0).getDate(); d++) {
      dates.push({
        date: window.moment(new Date([y, m, d])).format('YYYY-MM-DD'),
  ...
  for (let i = 0; i < dates.length; i++) {
    const week = Object.keys(weeks).length;
    const dateInfo = dates[i];
    if (!weeks[week]) {
      if (dateInfo.day === 0) weeks[week + 1] = [dateInfo.date];
      else {
        weeks[week + 1] = [];
        for (let p = 0; p <= dateInfo.day; p++) {
          weeks[week + 1].push(window.moment(dateInfo.date).startOf('week').add(p, 'd').format('YYYY-MM-DD'));
        }
      }
    } else {
      if (dateInfo.day === 0) {
        weeks[week + 1] = [dateInfo.date];
        if (i === dates.length - 1) {
          for (let p = 1; p <= 6 - dateInfo.day; p++) {
            weeks[week + 1].push(window.moment(dateInfo.date).add(p, 'd').format('YYYY-MM-DD'));
          }
        }
      } else {
        weeks[week].push(dateInfo.date);
        if (i === dates.length - 1) {
          for (let p = 1; p <= 6 - dateInfo.day; p++) {
            weeks[week].push(window.moment(dateInfo.date).add(p, 'd').format('YYYY-MM-DD'));
          }
        }
      }
    }
